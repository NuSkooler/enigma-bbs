...
describe('ML ACS code', () => {
    ...
    it('terminates on a band that refers back to ML', () => {
        ...
        //
        //  Recursion here is synchronous and unbounded, so without the
        //  guard this test would hang the suite rather than fail it --
        //  mocha's timeout cannot interrupt a spinning stack. Refuse the
        //  second entry so the failure is a failure.
        //
        let depth = 0;
        const realGetConditionalValue = client.acs.getConditionalValue.bind(client.acs);
        client.acs.getConditionalValue = (...args) => {
            if (++depth > 1) {
                depth -= 1;
                throw new Error('band evaluation recursed');
            }
            try {
                return realGetConditionalValue(...args);
            } finally {
                depth -= 1;
            }
        };
