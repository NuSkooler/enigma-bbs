...
//  Stub Events to avoid full system initialisation.
...
describe('User.create()', function () {
    ...
    it('stores password salt and dk as properties', done => {
        createUser(uniqueName(), 'password123', (err, user) => {
            assert.ifError(err);
            assert.ok(user.properties[UserProps.PassPbkdf2Salt], 'salt should be set');
    ...
    it('first user (root) gets userId=1 and sysops group', done => {
        createUser(uniqueName(), 'rootpass', (err, user) => {
            ...
            assert.ok(user.groups.includes('sysops'), 'root user should be in sysops');
            done();
        });
    });
