...
The figure is whole minutes, because that is the unit the budget is kept in -- time is billed a minute at a time. A door told "5 minutes" therefore has between four and five real minutes, depending where in the current minute it started. ENiGMA½ does not shave a minute off to hide that, the way current WWIV does, because the pre-check that decides whether a door may start at all works from the same number and the two would then disagree.
