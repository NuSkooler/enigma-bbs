...
function _loginHandler(req, resp, webServer, log) {
    ...
    parseJsonBody(req, (err, body) => {
        if (err || !body?.username || !body?.password) {
            return problemDetail(resp, 400, 'Bad Request', 'username and password are required');
        }
        ...
        User.getUserByUsername(username, (err, user) => {
            if (err || !user) {
                ...
                return problemDetail(resp, 401, 'Authentication Failed', 'Invalid credentials');
            }
            ...
            user.authenticateFactor1({ username, password }, authErr => {
                ...
                issueTokenPair(user.userId, user.username, user.groups || [], (err, tokens) => {
                if (err) {
                    log.error({ err }, 'Failed to issue token pair');
                    return problemDetail(resp, 500, 'Internal Server Error', 'Failed to create session');
                }
                ...
                resp.setHeader(
                    ...
                    log.info({ userId: user.userId, username: user.username }, 'REST API login success');
