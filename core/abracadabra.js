        ...
        async.series(
            [
                ...
                function validateTimeRemaining(callback) {
                    //
                    //  Refuse to *start* a door there is not time for: the
                    //  drop file states the budget and the door is expected
                    //  to honour it, which is what PCBoard and Maximus do.
                    //  Nothing checks the budget from inside a door -- but
                    //  the time-up kick disconnects, and door.js kills the
                    //  pty on ClientDisconnected, so a door caught running
                    //  when the clock runs out dies with the session. That
                    //  is exactly what this pre-check exists to avoid.
                    //
